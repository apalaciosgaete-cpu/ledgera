...
const categories = [
  ...
  { label: "Planes", color: "var(--warn)", faqs: [{ q: "¿Existe plan gratuito?", a: "Sí. El plan gratuito permite explorar la plataforma con un volumen limitado de operaciones." }, { q: "¿Qué plan conviene para una persona?", a: "El plan Personal está pensado para usuarios que necesitan ordenar operaciones, revisar portafolio, conciliación y reportes." }, { q: "¿Qué agrega el plan Profesional?", a: "Agrega herramientas de revisión técnica, auditoría ampliada, reportes trazables y soporte prioritario para operaciones más complejas." }] },
];
          ...
          <div className="grid gap-10 py-16">
            {categories.map((category) => (
              <section key={category.label}>
                <div className="mb-5 flex items-center gap-3"><span className="h-3 w-3 rounded-full" style={{ background: category.color }} /><h2 className="m-0 text-xs font-black uppercase tracking-[0.22em]" style={{ color: category.color }}>{category.label}</h2></div>
                <div className="grid gap-3">
                  {category.faqs.map((faq) => (
                    <details key={faq.q} className="group rounded-2xl border border-border bg-bg-elev p-5">
                      <summary className="cursor-pointer list-none font-display text-lg font-black tracking-[-0.025em] text-text marker:hidden">{faq.q}</summary>
                      <p className="mt-4 text-sm leading-7 text-text-soft">{faq.a}</p>
                    </details>
                  ))}
                </div>
              </section>
