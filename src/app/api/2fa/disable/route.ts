...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user?.twoFactorSecret) {
      return NextResponse.json({ ok: false, message: "2FA no configurado" }, { status: 400 });
    }
    ...
    return NextResponse.json({ ok: true, message: "2FA desactivado correctamente." });
  } catch (error) {
    console.error("[2fa/disable]", error);
    return NextResponse.json({ ok: false, message: "Error al desactivar 2FA" }, { status: 500 });
  }
}