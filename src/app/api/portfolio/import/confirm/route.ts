...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!normalized) { errorsMap[i] = "No se pudo normalizar"; continue; }
      const errors = validateMovement(normalized);
      if (errors.length > 0) { errorsMap[i] = errors.join(", "); continue; }
      normalizedRows.push({ index: i, data: normalized });
    }

    const existingMovements = await prisma.portfolioMovement.findMany({
      where: { userId: auth.user.id },
      select: { type: true, symbol: true, quantity: true, executedAt: true },
    });

    const simulation: SimulationMovement[] = [];

    for (const movement of existingMovements) {
      if (movement.type !== "BUY" && movement.type !== "SELL") continue;
      simulation.push({
        type:      movement.type,
    ...
    for (const movement of simulation) {
      ...
      if (movement.type === "BUY") {
        ...
        if (inventory[movement.symbol] < 0 && movement.source === "IMPORT") {
          errorsMap[movement.importIndex!] = "Venta sin inventario suficiente (rompe FIFO)";
        }
      }
    }
    ...
    for (const row of normalizedRows) {
      if (errorsMap[row.index]) {
        results.push({ index: row.index, ok: false, error: errorsMap[row.index] });
        continue;
      }
      ...
      if (exists) {
        results.push({ index: row.index, ok: false, error: "Movimiento duplicado" });
        continue;
      }
      ...
      results.push({ index: row.index, ok: true, id: created.id });
    }
    ...
    if (insertedCount > 0) {
      ...
      if (!rebuildResult.ok) {
        return fail("Importación ejecutada, pero falló la reconstrucción de eventos tributarios", 500);
      }
    }
    ...
    return ok(
      {
        ...
        rebuild: rebuildResult ? { ok: rebuildResult.ok, data: rebuildResult.data } : null,
      },
      insertedCount > 0
