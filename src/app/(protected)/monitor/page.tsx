  ...
  return (
    <main style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gap: 24 }}>
      ...
      <section style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 18, padding: 22 }}>
        <h2 style={{ color: "#0F2A3D", fontSize: 20, fontWeight: 900, margin: "0 0 14px" }}>
          Señales activas
        </h2>

        {signals.length === 0 ? (
          <div style={{ background: "#ECFDF5", border: "1px solid #BBF7D0", borderRadius: 14, padding: 18 }}>
            <p style={{ color: "#166534", fontSize: 16, fontWeight: 850, margin: "0 0 4px" }}>Sin señales críticas</p>
            <p style={{ color: "#166534", fontSize: 14, margin: 0 }}>No se detectan riesgos urgentes en este momento.</p>
          </div>
        ) : (
          <div style={{ display: "grid", gap: 12 }}>
            {signals.map((signal) => (
              <article key={signal.id} style={{ border: "1px solid #E2E8F0", borderRadius: 14, padding: 16, display: "grid", gap: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start" }}>
                  <div>
                    <p style={{ color: colorFor(signal.severity), fontSize: 12, fontWeight: 900, margin: "0 0 5px", textTransform: "uppercase" }}>
                      {signal.severity}
                    </p>
                    <h3 style={{ color: "#0F2A3D", fontSize: 17, fontWeight: 900, margin: 0 }}>{signal.title}</h3>
                    <p style={{ color: "#64748B", fontSize: 14, lineHeight: 1.55, margin: "6px 0 0" }}>{signal.description}</p>
                  </div>
                  <p style={{ color: "#0F2A3D", fontSize: 22, fontWeight: 900, margin: 0 }}>{signal.value ?? "—"}</p>
                </div>
                <Link href={signal.actionHref} style={{ color: "#0F766E", fontSize: 14, fontWeight: 850, textDecoration: "none" }}>
                  {signal.actionLabel} →
                </Link>
              </article>
            ))}
