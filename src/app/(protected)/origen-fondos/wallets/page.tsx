...
import { WALLETS, getHotWallets, getColdWallets, type SourceFundsItem } from "@/modules/crypto/catalogs/sourceFundsCatalogs";

type AssistantStatus = "idle" | "listening" | "speaking";

function renderWalletGroup(title: string, subtitle: string, wallets: SourceFundsItem[], router: ReturnType<typeof useRouter>) {
  if (wallets.length === 0) return null;
  return (
    <div>
      <div style={{ marginBottom: 8 }}>
        <strong style={{ display: "block", color: "#0F2A3D", fontSize: 14, fontWeight: 900, fontFamily: fonts.display }}>{title}</strong>
        <span style={{ color: "#64748B", fontSize: 11.5, fontFamily: fonts.body }}>{subtitle}</span>
      </div>
  ...
  return (
    <main style={{ height: "calc(100vh - 100px)", overflow: "hidden", display: "grid", gap: 8, gridTemplateRows: "auto 1fr auto" }}>
      <section>
        ...
        <p style={{ color: "#334155", fontSize: 12.5, lineHeight: 1.2, margin: 0, fontFamily: fonts.body }}>{WALLETS.length} wallets disponibles, separadas en calientes y frías. Selecciona para conectar o analizar dirección.</p>
      </section>

      <section style={{ minHeight: 0, overflowY: "auto", display: "flex", flexDirection: "column", gap: 16 }}>
        {renderWalletGroup("🔥 Wallets Calientes", "Conectadas a internet (extensión, móvil o web).", getHotWallets(), router)}
        {renderWalletGroup("❄️ Wallets Frías", "Almacenamiento en frío / hardware, sin conexión permanente.", getColdWallets(), router)}
      </section>
