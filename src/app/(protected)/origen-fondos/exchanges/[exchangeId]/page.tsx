    ...
    <main style={{ height: "calc(100vh - 100px)", overflow: "hidden", display: "grid", gap: 14, gridTemplateRows: "auto 1fr auto" }}>
      <section>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ margin: "4px 0 0", color: "var(--text)", fontSize: 13.5, fontFamily: fonts.body }}>Exchange</p>
          </div>
        </div>
      </section>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 14, alignContent: "start" }}>
        {exchange.connectionMethods.map((method) => (
          <button key={method} type="button" style={{ height: 150, borderRadius: 20, border: "1px solid var(--accent-soft)", background: "var(--bg-elev)", color: "var(--text)", cursor: "pointer", padding: 18, textAlign: "left", boxShadow: "0 10px 22px rgba(15,42,61,0.05)", fontFamily: fonts.body }}>
            <strong style={{ display: "block", fontSize: 17, fontWeight: 900, marginBottom: 8 }}>{method === "api" ? "Conexión por API" : "Subir historial"}</strong>
            <span style={{ display: "block", color: "var(--text)", fontSize: 13.5, lineHeight: 1.35 }}>{method === "api" ? "Conecta el exchange usando credenciales API de solo lectura." : "Carga CSV, Excel o reporte del exchange para normalizar movimientos."}</span>
          </button>
        ))}
      </section>

      <section style={{ border: "1px solid var(--accent-soft)", borderRadius: 18, background: "var(--bg-elev)", padding: 14, color: "var(--text)", fontSize: 13, fontFamily: fonts.body }}>
