  ...
  return (
    <main style={{ height: "calc(100vh - 100px)", overflow: "hidden", display: "grid", gap: 8, gridTemplateRows: "auto 1fr auto" }}>
      <section>
        ...
        <p style={{ color: "var(--text)", fontSize: 12.5, lineHeight: 1.2, margin: 0, fontFamily: fonts.body }}>{EXCHANGES.length} exchanges disponibles. Selecciona para conectar tu cuenta.</p>
      </section>
      ...
      <section style={{ width: "100%", border: "1px solid var(--accent-soft)", borderRadius: 18, background: "var(--bg-elev)", padding: 12, display: "grid", gap: 8, boxShadow: "0 10px 22px rgba(109,74,255,0.05)", boxSizing: "border-box" }}>
        ...
        <form onSubmit={submit} style={{ width: "100%" }}>
          <div style={{ flex: 1, minHeight: 44, borderRadius: 15, border: "1px solid var(--border)", background: "var(--bg-elev)", display: "flex", alignItems: "center", padding: "0 14px", gap: 6, minWidth: 0, boxShadow: "0 6px 14px rgba(15,42,61,0.035)" }}>
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Escribe el nombre de tu exchange..." style={{ flex: 1, border: "none", outline: "none", color: "var(--text)", fontSize: 14, fontFamily: fonts.body, minWidth: 0, background: "transparent" }} />
          </div>
        </form>
      </section>
    </main>
