  ...
  return (
    <main style={{ maxWidth: 1120, margin: "0 auto", display: "grid", gap: 24 }}>
      ...
      <section style={{ background: "var(--bg-elev)", border: "1px solid var(--border)", borderRadius: 18, padding: 22 }}>
        <h2 style={{ color: "var(--text)", fontSize: 20, fontWeight: 900, margin: "0 0 14px" }}>Historial</h2>
        {executions.length === 0 ? (
          <p style={{ color: "var(--text-soft)", fontSize: 14, margin: 0 }}>Aún no hay ejecuciones registradas.</p>
        ) : (
          <div style={{ display: "grid", gap: 12 }}>
            {executions.map((execution) => (
              <article key={execution.id} style={{ border: "1px solid var(--border)", borderRadius: 14, padding: 16, display: "grid", gap: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                  <div>
                    ...
                    <h3 style={{ color: "var(--text)", fontSize: 17, fontWeight: 900, margin: 0 }}>{execution.title}</h3>
                    <p style={{ color: "var(--text-soft)", fontSize: 14, lineHeight: 1.55, margin: "6px 0 0" }}>{execution.description}</p>
                  </div>
                  <p style={{ color: "var(--text-soft)", fontSize: 12, fontWeight: 700, margin: 0 }}>{execution.sourceType ?? "Manual"}</p>
                </div>
                {execution.error && <p style={{ color: "var(--loss)", fontSize: 13, fontWeight: 700, margin: 0 }}>{execution.error}</p>}
              </article>
            ))}
          </div>
        )}
...
function colorFor(status: Execution["status"]) {
  if (status === "COMPLETED") return "#3FA687";
  if (status === "FAILED" || status === "REJECTED") return "#C4634A";
  if (status === "RUNNING") return "#3FA687";
  return "#E8B84B";
}
