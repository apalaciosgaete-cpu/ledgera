  ...
  if (loading) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "40vh" }}>
        <p style={{ color: "#94A3B8", fontSize: 16 }}>Cargando panel de casos...</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "grid", gap: 24 }}>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14 }}>
        <KpiCard label="Usuarios afectados" value={data?.totalUsers ?? 0} detail="Con casos activos" color="#2563EB" />
        <KpiCard label="Casos críticos" value={data?.criticalCount ?? 0} detail="Requieren acción" color="#B91C1C" />
        <KpiCard label="Casos abiertos" value={data?.openCount ?? 0} detail="En curso" color="#B45309" />
        <KpiCard label="Casos resueltos" value={data?.resolvedCount ?? 0} detail="Gestionados" color="#047857" />
      </section>
      ...
      {items.length === 0 ? (
        <section style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 18, padding: 32, textAlign: "center" }}>
          <p style={{ color: "#64748B", fontSize: 15, margin: 0 }}>No se encontraron casos con los filtros seleccionados.</p>
        </section>
      ) : (
        <section style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 18, overflow: "hidden" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              <thead>
                <tr style={{ background: "#F8FAFC", borderBottom: "1px solid #E2E8F0" }}>
                  <th style={{ padding: "12px 16px", textAlign: "left", color: "#64748B", fontWeight: 800, fontSize: 11, textTransform: "uppercase" }}>Prioridad</th>
                  <th style={{ padding: "12px 16px", textAlign: "left", color: "#64748B", fontWeight: 800, fontSize: 11, textTransform: "uppercase" }}>Caso</th>
                  <th style={{ padding: "12px 16px", textAlign: "left", color: "#64748B", fontWeight: 800, fontSize: 11, textTransform: "uppercase" }}>Usuario</th>
                  <th style={{ padding: "12px 16px", textAlign: "left", color: "#64748B", fontWeight: 800, fontSize: 11, textTransform: "uppercase" }}>Estado</th>
                  <th style={{ padding: "12px 16px", textAlign: "left", color: "#64748B", fontWeight: 800, fontSize: 11, textTransform: "uppercase" }}>Origen</th>
                  <th style={{ padding: "12px 16px", textAlign: "left", color: "#64748B", fontWeight: 800, fontSize: 11, textTransform: "uppercase" }}>Actualización</th>
                </tr>
              </thead>
              <tbody>
                {items.map((tc) => (
                  <tr key={tc.id} style={{ borderBottom: "1px solid #F1F5F9" }}>
                    ...
                    <td style={{ padding: "14px 16px", color: "#0F2A3D", fontWeight: 700, maxWidth: 280 }}>
                      <p style={{ margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tc.title}</p>
                    </td>
                    <td style={{ padding: "14px 16px", color: "#64748B", fontSize: 13 }}>{tc.userId.slice(0, 8)}...</td>
                    <td style={{ padding: "14px 16px" }}>
                      <span style={{
                        display: "inline-block",
                        padding: "2px 10px",
          ...
          <div style={{ padding: "12px 22px", borderTop: "1px solid #E2E8F0", textAlign: "center" }}>
