    ...
    <div style={{ maxWidth: 1180, width: "100%" }}>
      <section style={{ alignItems: "flex-start", display: "flex", flexWrap: "wrap", gap: 16, justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          ...
          <h1 style={{ color: "var(--text)", fontSize: "1.85rem", fontWeight: 850, lineHeight: 1.12, margin: "0 0 8px" }}>Verificaciones</h1>
      ...
      {tab === "evidencia" && (
        <>
          {evLoading ? (
            <p style={{ color: "var(--text-soft)", fontSize: 14, fontWeight: 750 }}>Cargando evidencia…</p>
          ) : evError ? (
            <div style={{ background: "rgba(196,99,74,0.14)", border: "1px solid rgba(196,99,74,0.14)", borderRadius: 8, color: "var(--loss)", fontWeight: 750, padding: 16 }}>{evError}</div>
          ) : evData ? (
            <>
              {evData.total > 0 && (
                <section style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", marginBottom: 24 }}>
                  <article style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: 16, textAlign: "center" }}>
                    <p style={{ color: "var(--text-soft)", fontSize: 11, fontWeight: 850, letterSpacing: "0.04em", margin: "0 0 8px", textTransform: "uppercase" }}>Total</p>
                    <p style={{ color: "var(--text)", fontSize: "1.6rem", fontWeight: 850, lineHeight: 1.15, margin: 0 }}>{evData.total}</p>
                  </article>
                  <article style={{ background: "rgba(22,163,74,0.08)", border: "1px solid rgba(22,163,74,0.2)", borderRadius: 8, padding: 16, textAlign: "center" }}>
                    ...
                    <p style={{ color: "var(--accent)", fontSize: "1.6rem", fontWeight: 850, lineHeight: 1.15, margin: 0 }}>{evData.confirmedCount}</p>
                  </article>
                  <article style={{ background: "rgba(220,38,38,0.06)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 8, padding: 16, textAlign: "center" }}>
                    <p style={{ color: "var(--text-soft)", fontSize: 11, fontWeight: 850, letterSpacing: "0.04em", margin: "0 0 8px", textTransform: "uppercase" }}>Anuladas</p>
                    <p style={{ color: "var(--loss)", fontSize: "1.6rem", fontWeight: 850, lineHeight: 1.15, margin: 0 }}>{evData.voidedCount}</p>
                  </article>
                </section>
              )}
              ...
              {evData.evidence.length === 0 ? (
                <section style={{ background: "rgba(255,255,255,0.04)", border: "1px dashed rgba(255,255,255,0.12)", borderRadius: 8, padding: 28, textAlign: "center" }}>
                  <h2 style={{ color: "var(--text)", fontSize: "1.15rem", fontWeight: 850, margin: "0 0 8px" }}>Sin evidencia registrada</h2>
                  <p style={{ color: "var(--text-soft)", fontSize: 14, lineHeight: 1.55, margin: "0 auto", maxWidth: 520 }}>Las declaraciones se generan automáticamente. Cuando existan, podrás ver su hash y verificar su integridad públicamente.</p>
                </section>
              ) : (
                <>
                    ...
                    {evData.evidence.map((item) => (
                      <article key={item.id} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: 20 }}>
                        <div style={{ alignItems: "center", display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 10 }}>
                          <h3 style={{ color: "var(--text)", fontSize: "1rem", fontWeight: 850, margin: 0 }}>{item.type} — {item.taxYear}</h3>
                        ...
                        <div style={{ background: "var(--bg-sunken)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, marginBottom: 12, padding: "10px 12px" }}>
                          <p style={{ color: "var(--text-soft)", fontSize: 11, fontWeight: 850, letterSpacing: "0.04em", margin: "0 0 4px", textTransform: "uppercase" }}>Hash de integridad</p>
                          <div style={{ alignItems: "center", display: "flex", flexWrap: "wrap", gap: 8 }}>
                            <code style={{ color: "var(--text)", fontFamily: "monospace", fontSize: 13 }}>{item.hashShort}</code>
                            <button
                              type="button"
                              onClick={() => copyToClipboard(item.hash, item.id)}
                              style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, color: "var(--text-faint)", cursor: "pointer", fontSize: 12, fontWeight: 750, padding: "4px 10px" }}
                            >
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                          <a
                            ...
                            style={{ background: "var(--accent)", borderRadius: 6, color: "var(--text)", display: "inline-flex", fontSize: 13, fontWeight: 850, padding: "8px 14px", textDecoration: "none" }}
                          >
                          ...
                          <button
                            ...
                            style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, color: "var(--text-faint)", cursor: "pointer", fontSize: 13, fontWeight: 750, padding: "8px 14px" }}
                          >
                  ...
                  <section style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: 18 }}>
                    <h2 style={{ color: "var(--text)", fontSize: "1rem", fontWeight: 850, margin: "0 0 10px" }}>¿Cómo funciona la verificación?</h2>
                    <ol style={{ color: "var(--text-soft)", fontSize: 14, lineHeight: 1.6, margin: 0, paddingLeft: 18 }}>
      ...
      {tab === "reportes" && (
        <>
          <section style={{ alignItems: "end", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", marginBottom: 20, padding: 16 }}>
            <label style={{ color: "var(--text-soft)", display: "grid", fontSize: 13, fontWeight: 750, gap: 6 }}>
              Año
              <select value={valYear} onChange={(e) => setValYear(Number(e.target.value))} style={{ background: "var(--bg-sunken)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "var(--text)", minHeight: 40, padding: "0 10px" }}>
          ...
          {valLoading && <p style={{ color: "var(--text-soft)", fontSize: 14, fontWeight: 750 }}>Cargando verificaciones…</p>}
          {valError && <div style={{ background: "rgba(196,99,74,0.14)", border: "1px solid rgba(196,99,74,0.14)", borderRadius: 8, color: "var(--loss)", fontWeight: 750, padding: 16 }}>{valError}</div>}

          {!valLoading && valData && (
            <>
              {valData.validations.length === 0 ? (
                <section style={{ background: "rgba(255,255,255,0.04)", border: "1px dashed rgba(255,255,255,0.12)", borderRadius: 8, padding: 28, textAlign: "center" }}>
                  <h2 style={{ color: "var(--text)", fontSize: "1.15rem", fontWeight: 850, margin: "0 0 8px" }}>Sin verificaciones para {valYear}</h2>
                  <p style={{ color: "var(--text-soft)", fontSize: 14, lineHeight: 1.55, margin: 0 }}>Las verificaciones se generan al descargar reportes con hash.</p>
                </section>
              ) : (
                        ...
                        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap" }}>
                          <div style={{ flex: 1, minWidth: "200px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
                              <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: "4px", fontSize: "0.6875rem", fontWeight: 700, background: v.isValid ? "rgba(22,163,74,0.12)" : "rgba(220,38,38,0.12)", color: v.isValid ? "var(--accent)" : "var(--loss)" }}>
                                {v.isValid ? "Válido" : "Revocado"}
                              </span>
                              <span style={{ fontSize: "0.875rem", fontWeight: 600, color: "var(--text)" }}>{v.typeLabel}</span>
                            </div>
                            <p style={{ margin: "0 0 4px", fontSize: "0.75rem", color: "var(--text-soft)" }}>Emitido: {new Date(v.issuedAt).toLocaleDateString("es-CL")}{v.symbol ? ` · Activo: ${v.symbol}` : ""} · Año: {v.year}</p>
                            <p style={{ margin: 0, fontFamily: "monospace", fontSize: "0.75rem", color: "var(--text-soft)", wordBreak: "break-all" }}>{v.hash}</p>
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", gap: "6px", flexShrink: 0 }}>
                            <a href={verifyUrl} target="_blank" rel="noopener noreferrer" style={{ display: "inline-block", padding: "0.4rem 1rem", borderRadius: "8px", background: "var(--bg-elev)", color: "var(--text)", fontSize: "0.8125rem", fontWeight: 600, textDecoration: "none", textAlign: "center", whiteSpace: "nowrap" }}>
                              Ver verificación
                            </a>
                            <button onClick={() => copyLink(v.id, verifyUrl)} style={{ background: copiedId === v.id ? "rgba(22,163,74,0.12)" : "rgba(255,255,255,0.06)", border: `1px solid ${copiedId === v.id ? "rgba(22,163,74,0.3)" : "rgba(255,255,255,0.08)"}`, borderRadius: "8px", padding: "0.4rem 1rem", fontSize: "0.8125rem", fontWeight: 600, color: copiedId === v.id ? "var(--accent)" : "var(--text-soft)", cursor: "pointer", whiteSpace: "nowrap" }}>
