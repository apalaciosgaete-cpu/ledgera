...
const ACTION_CONFIG: Record<string, { label: string; description: string; color: string; bg: string; icon: string }> = {
  CLOSE: { label: "Período cerrado", description: "Se congeló el estado tributario del período.", color: "#64748B", bg: "rgba(100,116,139,0.08)", icon: "🔒" },
  REOPEN: { label: "Período reabierto", description: "Se habilitó el período para realizar correcciones.", color: "#D97706", bg: "rgba(245,158,11,0.08)", icon: "🔓" },
  SNAPSHOT: { label: "Registro de cierre generado", description: "Se creó un respaldo del estado tributario.", color: "#0F2A3D", bg: "rgba(15,42,61,0.06)", icon: "📋" },
...
function statusConfig(status: PeriodStatus) {
  switch (status) {
    case "OPEN": return { label: "Abierto", color: "#16A34A", bg: "#F0FDF4", border: "#86EFAC", icon: "◌" };
    case "CLOSED": return { label: "Cerrado", color: "#64748B", bg: "#F8FAFC", border: "#CBD5E1", icon: "🔒" };
    case "REOPENED": return { label: "Reabierto", color: "#D97706", bg: "#FFFBEB", border: "#FCD34D", icon: "🔓" };
  }
}
    ...
    <div>
      ...
      <section style={{ alignItems: "end", background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 8, display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", marginBottom: 20, padding: 16 }}>
        <label style={{ color: "#475569", display: "grid", fontSize: 13, fontWeight: 750, gap: 6 }}>
          Año tributario
          <select value={year} onChange={(e) => { const y = Number(e.target.value); setYear(y); void load(y); }} style={{ border: "1px solid #CBD5E1", borderRadius: 8, color: "#0F2A3D", minHeight: 40, padding: "0 10px" }}>
        ...
        <>
          ...
          <section style={{ background: cfg.bg, border: `2px solid ${cfg.border}`, borderRadius: 12, marginBottom: 20, padding: 22 }}>
            <div style={{ alignItems: "center", display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ background: "#FFFFFF", borderRadius: 999, color: cfg.color, display: "inline-flex", fontSize: 13, fontWeight: 850, padding: "6px 12px" }}>Período {data.year} — {cfg.label}</span>
            </div>
            ...
            <section style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 8, marginBottom: 20, overflow: "hidden" }}>
              ...
              {!data.readyToClose && data.status === "OPEN" && (
                <div style={{ background: "#FEF2F2", borderTop: "1px solid #FECACA", padding: "12px 18px" }}>
                  <p style={{ color: "#991B1B", fontSize: 13, fontWeight: 750, margin: 0 }}>⚠ No puedes cerrar el período hasta resolver los puntos marcados arriba.</p>
                </div>
              )}
            </section>
          )}
          ...
          <section style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", marginBottom: 20 }}>
            {[
              ...
              { label: "Ganancia / Pérdida", value: clp(data.summary.totalPnlClp), sub: `Realizado en ${year}`, color: data.summary.totalPnlClp >= 0 ? "#15803D" : "#B45309" },
              ...
              { label: "Salud datos", value: `${data.summary.healthScore}/100`, sub: "Score tributario", color: data.summary.healthScore >= 70 ? "#15803D" : data.summary.healthScore >= 40 ? "#B45309" : "#991B1B" },
            ].map((item) => (
              <article key={item.label} style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 8, padding: 18 }}>
                <p style={{ color: "#64748B", fontSize: 11, fontWeight: 850, letterSpacing: "0.04em", margin: "0 0 8px", textTransform: "uppercase" }}>{item.label}</p>
                <p style={{ color: item.color ?? "#0F2A3D", fontSize: "1.45rem", fontWeight: 850, lineHeight: 1.15, margin: "0 0 6px" }}>{item.value}</p>
                <p style={{ color: "#64748B", fontSize: 13, margin: 0 }}>{item.sub}</p>
              </article>
            ))}
          </section>
          ...
          {data.status === "OPEN" && data.readyToClose && (
            <section style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 8, marginBottom: 20, padding: "18px 20px" }}>
              ...
              <p style={{ color: "#64748B", fontSize: 13, lineHeight: 1.55, margin: "0 0 14px" }}>Todo está en orden. Al cerrar, se congelarán tus movimientos y eventos tributarios.</p>
              <div style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 260 }}>
                  <label style={{ color: "#475569", display: "block", fontSize: 12, fontWeight: 750, marginBottom: 6 }}>Motivo de cierre</label>
                  <input type="text" value={closeReason} onChange={(e) => { setCloseReason(e.target.value); setCloseError(""); setCloseSuccess(""); }} placeholder="Ej: Cierre período anual para declaración SII" style={{ border: "1px solid #CBD5E1", borderRadius: 8, color: "#0F2A3D", fontSize: 14, padding: "10px 12px", width: "100%", boxSizing: "border-box" }} />
                </div>
                <button onClick={handleClose} disabled={closeLoading} style={{ background: closeLoading ? "#94A3B8" : "#0F2A3D", border: "none", borderRadius: 8, color: "#FFFFFF", cursor: closeLoading ? "not-allowed" : "pointer", fontSize: 14, fontWeight: 850, padding: "10px 18px", whiteSpace: "nowrap" }}>{closeLoading ? "Cerrando…" : "Cerrar período"}</button>
              </div>
          ...
          {data.status === "CLOSED" && (
            <section style={{ background: "#FFFBEB", border: "1px solid #FCD34D", borderRadius: 8, marginBottom: 20, padding: "18px 20px" }}>
              ...
              <p style={{ color: "#78350F", fontSize: 13, lineHeight: 1.55, margin: "0 0 14px" }}>El período está cerrado. Si necesitas corregir movimientos o eventos, reábrelo.</p>
              <div style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 260 }}>
                  <label style={{ color: "#475569", display: "block", fontSize: 12, fontWeight: 750, marginBottom: 6 }}>Motivo de reapertura</label>
                  <input type="text" value={reopenReason} onChange={(e) => { setReopenReason(e.target.value); setReopenError(""); setReopenSuccess(""); }} placeholder="Ej: Corrección de clasificación de eventos" style={{ border: "1px solid #CBD5E1", borderRadius: 8, color: "#0F2A3D", fontSize: 14, padding: "10px 12px", width: "100%", boxSizing: "border-box" }} />
                </div>
                <button onClick={handleReopen} disabled={reopenLoading} style={{ background: reopenLoading ? "#94A3B8" : "#D97706", border: "none", borderRadius: 8, color: "#FFFFFF", cursor: reopenLoading ? "not-allowed" : "pointer", fontSize: 14, fontWeight: 850, padding: "10px 18px", whiteSpace: "nowrap" }}>{reopenLoading ? "Reabriendo…" : "Reabrir período"}</button>
              </div>
          ...
          {data.status === "REOPENED" && (
            <section style={{ background: "#FEF9C3", border: "1px solid #FDE047", borderRadius: 8, marginBottom: 20, padding: "14px 18px" }}>
              <p style={{ color: "#854D0E", fontSize: 13, fontWeight: 750, margin: 0 }}>⚠ El período está reabierto. Realiza las correcciones necesarias y luego ciérralo nuevamente.</p>
            </section>
          )}
          ...
          {auditData && (
            <>
              <section style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 8, marginBottom: 20, overflow: "hidden" }}>
                <div style={{ padding: "16px 18px", borderBottom: "1px solid #E2E8F0" }}>
                  <h3 style={{ color: "#0F2A3D", fontSize: "1rem", fontWeight: 850, margin: 0 }}>Historial de acciones</h3>
                  <p style={{ color: "#64748B", fontSize: 13, margin: "4px 0 0" }}>{auditData.logs.length} eventos registrados para {year}</p>
                </div>
                {auditData.logs.length === 0 ? (
                  <div style={{ padding: "2rem", textAlign: "center" }}>
                    <p style={{ color: "#94A3B8", fontWeight: 600, margin: "0 0 4px" }}>Sin actividad registrada</p>
                  </div>
                ) : (
                      ...
                      return (
                        <div key={log.id} style={{ display: "flex", gap: "1rem", paddingBottom: isLast ? 0 : "1.25rem" }}>
                          ...
                          <div style={{ flex: 1, paddingTop: "4px" }}>
                            ...
                            <p style={{ margin: 0, fontSize: "0.8125rem", color: "#475569" }}>{actionCfg.description}{log.reason ? ` Motivo: "${log.reason}".` : ""}</p>
                            {log.actorEmail && <p style={{ margin: "4px 0 0", fontSize: "0.75rem", color: "#94A3B8" }}>Por: {log.actorEmail}</p>}
                          </div>
                        </div>
                      );
                    })}
              ...
              {auditData.registros.length > 0 && (
                <section style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 8, marginBottom: 20, overflow: "hidden" }}>
                  <div style={{ padding: "16px 18px", borderBottom: "1px solid #E2E8F0" }}>
                    <h3 style={{ color: "#0F2A3D", fontSize: "1rem", fontWeight: 850, margin: 0 }}>Constancias de estado tributario</h3>
                    <p style={{ color: "#64748B", fontSize: 13, margin: "4px 0 0" }}>Registro técnico del estado del portafolio al momento de cada cierre</p>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px", padding: "1rem 1.25rem" }}>
                    {auditData.registros.map((reg, index) => (
                      <div key={reg.id} style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 10, padding: "0.875rem 1.25rem", display: "flex", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
          ...
          <section style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 8, padding: "14px 18px" }}>
            <div style={{ alignItems: "center", display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "space-between" }}>
              <p style={{ color: "#64748B", fontSize: 13, margin: 0 }}>¿Necesitas exportar trazabilidad?</p>
              <div style={{ display: "flex", gap: 10 }}>
                <a href={`/api/tax/periods/audit/export/csv?year=${year}`} download={`ledgera-trazabilidad-${year}.csv`} style={{ background: "#FFFFFF", border: "1px solid #CBD5E1", borderRadius: 8, color: "#0F2A3D", display: "inline-flex", fontSize: 13, fontWeight: 850, padding: "10px 14px", textDecoration: "none", whiteSpace: "nowrap" }}>Trazabilidad CSV</a>
                <a href={`/api/tax/periods/audit/export/pdf?year=${year}`} download={`ledgera-trazabilidad-${year}.pdf`} style={{ background: "#0F2A3D", border: "1px solid #0F2A3D", borderRadius: 8, color: "#FFFFFF", display: "inline-flex", fontSize: 13, fontWeight: 850, padding: "10px 14px", textDecoration: "none", whiteSpace: "nowrap" }}>Trazabilidad PDF</a>
              </div>
            </div>
          </section>
        </>
