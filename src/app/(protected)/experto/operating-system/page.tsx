  ...
  if (loading) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "40vh" }}>
        <p style={{ color: "#94A3B8", fontSize: 16 }}>Cargando panel LAIOS...</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "grid", gap: 24 }}>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 14 }}>
        <Kpi label="Usuarios totales" value={data?.total ?? 0} color="#0F2A3D" />
        <Kpi label="🟢 Óptimo" value={dist.OPTIMAL ?? 0} color="#047857" />
        <Kpi label="🔵 Normal" value={dist.NORMAL ?? 0} color="#2563EB" />
        <Kpi label="🟡 Atención" value={dist.ATTENTION ?? 0} color="#B45309" />
        <Kpi label="🔴 Crítico" value={dist.CRITICAL ?? 0} color="#B91C1C" />
      </section>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              <thead>
                <tr style={{ background: "#F8FAFC", borderBottom: "1px solid #E2E8F0" }}>
                  ...
                  <th style={{ padding: "12px 16px", textAlign: "left", color: "#64748B", fontWeight: 800, fontSize: 11, textTransform: "uppercase" }}>Resumen Ejecutivo</th>
                  <th style={{ padding: "12px 16px", textAlign: "left", color: "#64748B", fontWeight: 800, fontSize: 11, textTransform: "uppercase" }}>Última Actualización</th>
                </tr>
              </thead>
              <tbody>
                {items.map((s) => {
                  ...
                  return (
                    <tr key={s.userId} style={{ borderBottom: "1px solid #F1F5F9" }}>
                      <td style={{ padding: "14px 16px", color: "#0F2A3D", fontWeight: 700 }}>
                        {s.userId.slice(0, 8)}...
                      </td>
                      ...
                      <td style={{ padding: "14px 16px", color: "#64748B", fontSize: 13, maxWidth: 400 }}>
