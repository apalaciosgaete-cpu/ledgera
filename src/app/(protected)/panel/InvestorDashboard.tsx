...
export function InvestorDashboard() {
  ...
  useEffect(() => {
    fetch("/api/copilot/conversations?limit=5", { cache: "no-store" })
      .then((r) => r.json())
      .then((json) => {
        if (json.ok && Array.isArray(json.data)) setConversations(json.data);
      })
      .catch(() => null)
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 60px)", background: "#071B28", color: "#E2E8F0", fontFamily: fonts.body, display: "flex", flexDirection: "column", alignItems: "center", padding: "clamp(40px, 7vh, 76px) 24px 40px" }}>
      <div style={{ width: "100%", maxWidth: 720, textAlign: "center" }}>
        <h1 style={{ color: "#F8FAFC", fontSize: "clamp(2.25rem, 6vw, 4rem)", fontWeight: 900, margin: "0 0 8px", lineHeight: 0.96, fontFamily: fonts.display, letterSpacing: "0.14em" }}>
          LEDGERA
        </h1>
        <p style={{ color: "#4ADE80", fontSize: "clamp(0.78rem, 2vw, 0.95rem)", fontWeight: 850, letterSpacing: "0.18em", textTransform: "uppercase", margin: "0 0 34px", fontFamily: fonts.body }}>
          SO Financiero y Tributario
        </p>

        <p style={{ color: "#64748B", fontSize: "clamp(1rem, 2.5vw, 1.28rem)", fontWeight: 450, margin: "0 0 22px", lineHeight: 1.45 }}>
          ¿Qué decisión relacionada con cryptoactivos necesitas evaluar?
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ marginBottom: 18 }}>
          <div style={{ display: "flex", alignItems: "center", background: "rgba(255,255,255,0.04)", border: `1.5px solid ${focused ? "rgba(74,222,128,0.35)" : "rgba(255,255,255,0.10)"}`, borderRadius: 18, padding: "6px 6px 6px 22px", transition: "border-color 0.2s", boxShadow: focused ? "0 0 0 3px rgba(22,163,74,0.08)" : "none" }}>
            <input
              ...
              autoComplete="off"
              style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: "#F8FAFC", fontSize: 16, fontFamily: fonts.body, fontWeight: 500, padding: "12px 0", caretColor: "#4ADE80" }}
            />
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
          {EXAMPLES.slice(0, 4).map((ex) => (
            <button key={ex} onClick={() => useExample(ex)} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 999, color: "#64748B", cursor: "pointer", fontSize: 13, fontWeight: 550, padding: "6px 14px", fontFamily: fonts.body }}>
              {ex}
            </button>
          ))}
        </div>
      ...
      <section style={{ width: "100%", maxWidth: 980, marginTop: 48 }}>
        <p style={{ color: "#334155", fontSize: 11, fontWeight: 850, letterSpacing: "0.09em", textTransform: "uppercase", margin: "0 0 16px", fontFamily: fonts.body }}>
          Patrimonio digital
        </p>
      ...
      <section style={{ width: "100%", maxWidth: 720, marginTop: 44 }}>
        ...
        {loadingConvs ? (
          <p style={{ color: "#475569", fontSize: 13, margin: 0 }}>Cargando...</p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {conversations.map((conv) => (
              <Link key={conv.id} href={`/conversaciones?conversationId=${conv.id}`} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, padding: "12px 18px", borderRadius: 12, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", textDecoration: "none" }}>
                <span style={{ color: "#CBD5E1", fontSize: 14, fontWeight: 600, fontFamily: fonts.body, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, textAlign: "left" }}>{conv.title}</span>
                <span style={{ color: "#475569", fontSize: 12, flexShrink: 0, fontFamily: fonts.body }}>{timeAgo(conv.lastMessageAt)}</span>
              </Link>
            ))}
          </div>
        )}
