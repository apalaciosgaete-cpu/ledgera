      ...
      <section style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 16, overflow: "hidden" }}>
        ...
        {data.posiciones.length === 0 ? (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" }}>
            <thead>
              <tr>{["Activo", "Cantidad", "Costo USD", "Costo CLP"].map((h) => <th key={h} style={{ textAlign: "left", padding: "0.85rem 1rem", color: "#64748B", fontSize: "0.75rem" }}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {data.posiciones.map((p) => (
                <tr key={p.symbol}>
                  <td style={{ padding: "0.85rem 1rem", fontWeight: 800 }}>{p.symbol}</td>
                  <td style={{ padding: "0.85rem 1rem" }}>{number(p.quantity)}</td>
                  <td style={{ padding: "0.85rem 1rem" }}>{usd(p.totalCostUsd)}</td>
                  <td style={{ padding: "0.85rem 1rem", fontWeight: 800 }}>{clp(p.totalCostClp)}</td>
                </tr>
              ))}
            </tbody>
          </table>
