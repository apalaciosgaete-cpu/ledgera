        ...
        <PublicContainer>
          <div className="grid gap-10 py-16 md:grid-cols-[1fr_320px]">
            <article className="space-y-6">
              ...
              {content.sources?.length ? <section className="rounded-3xl border border-border bg-bg-elev p-7"><h2 className="font-display text-2xl font-black tracking-[-0.03em] text-text">Fuentes oficiales</h2><p className="mt-4 text-base leading-8 text-text-soft">Referencias consultadas para mantener este contenido verificable y actualizado.</p><ul className="mt-5 grid gap-3 p-0">{content.sources.map((source) => <li key={source.href} className="list-none"><a href={source.href} target="_blank" rel="noopener noreferrer" className={linkCardClass}>{source.label}<span className="sr-only"> (abre en una pestaña nueva)</span></a></li>)}</ul></section> : null}
            </article>
