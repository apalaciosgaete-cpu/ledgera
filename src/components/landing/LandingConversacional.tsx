    ...
    <main className="min-h-screen overflow-x-hidden bg-[#080E1F] text-[#F2EBD8]">
      <nav className="sticky top-0 z-[100] border-b border-[#24345F]/70 bg-[#080E1F]/88 px-5 backdrop-blur-xl sm:px-8 lg:px-10">
        <div className="mx-auto flex min-h-[82px] max-w-[1440px] items-center justify-between gap-5">
          <Link href="/" aria-label="Inicio LEDGERA" className="inline-flex items-center"><Logo size="sm" /></Link>
          <div className="hidden items-center gap-7 lg:flex">
            {navItems.map(([label, href]) => <Link key={href} href={href} className="text-sm font-bold text-[#BFC8D9] transition hover:text-[#F2EBD8]">{label}</Link>)}
            <Link href="/login" className="text-sm font-black text-[#BFC8D9] transition hover:text-[#F2EBD8]">Ingresar</Link>
            <Link href="/register" className="inline-flex rounded-2xl bg-[#C9A84C] px-5 py-3 text-sm font-black text-[#080E1F] shadow-[0_18px_60px_rgba(201,168,76,0.22)] transition hover:-translate-y-0.5 hover:bg-[#DDBB61]">Comenzar</Link>
          </div>
          <button type="button" className="rounded-2xl border border-[#24345F] bg-[#0B1430] px-4 py-3 text-sm font-black text-[#F2EBD8] lg:hidden" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>Menú</button>
        </div>
      </nav>
      ...
      {mobileMenuOpen ? (
        <div className="sticky top-[82px] z-[90] border-b border-[#24345F] bg-[#080E1F]/98 px-6 py-5 backdrop-blur-xl lg:hidden">
          <div className="grid gap-4">
            {navItems.map(([label, href]) => <Link key={href} href={href} onClick={() => setMobileMenuOpen(false)} className="text-base font-black text-[#BFC8D9]">{label}</Link>)}
      ...
      <section className="relative overflow-hidden px-5 pb-12 pt-16 sm:px-8 lg:px-10 lg:pb-16 lg:pt-20">
        ...
        <div className="relative z-10 mx-auto grid max-w-[1440px] items-center gap-12 lg:grid-cols-[minmax(0,0.92fr)_minmax(560px,1fr)]">
          <div>
            <div className="inline-flex rounded-full border border-[#C9A84C]/50 bg-[#111D3E] px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-[#C9A84C]">Bienvenido a LEDGERA</div>
            <h1 className="mt-7 max-w-[820px] font-display text-[44px] font-black leading-[1.03] tracking-[-0.055em] text-[#F2EBD8] sm:text-[62px] lg:text-[76px]">Ordena tus activos, revisa tus obligaciones y genera respaldo tributario.</h1>
            <p className="mt-6 max-w-[720px] text-lg leading-8 text-[#BFC8D9] sm:text-xl">LEDGERA transforma movimientos de crypto, bancos e inversiones en información ordenada, trazable y exportable para revisar antes de declarar.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link href="/register" className="inline-flex min-h-[58px] items-center justify-center rounded-2xl bg-[#C9A84C] px-7 text-base font-black text-[#080E1F] shadow-[0_24px_80px_rgba(201,168,76,0.24)] transition hover:-translate-y-0.5 hover:bg-[#DDBB61]">Comenzar evaluación →</Link>
              <Link href="/opinion" className="inline-flex min-h-[58px] items-center justify-center rounded-2xl border border-[#24345F] bg-[#0B1430]/88 px-7 text-base font-black text-[#C9A84C] transition hover:border-[#C9A84C]/70 hover:bg-[#111D3E]">Ayúdanos a mejorar</Link>
            </div>
      ...
      <footer className="border-t border-[#24345F]/70 bg-[#070B18] px-5 py-12 text-[#BFC8D9] sm:px-8 lg:px-10">
        <div className="mx-auto grid max-w-[1440px] gap-10 lg:grid-cols-[1.1fr_1.5fr_0.8fr]">
          <div>
            <Logo size="sm" />
            <p className="mt-5 max-w-[430px] text-sm leading-6 text-[#7F8BA5]">Orden, trazabilidad y respaldo tributario para activos financieros. Diseñado para revisar mejor antes de declarar o tomar decisiones patrimoniales.</p>
          </div>
          <div className="grid gap-8 sm:grid-cols-3">
            {footerColumns.map((column) => (
              <div key={column.title}>
                <h3 className="text-sm font-black uppercase tracking-[0.18em] text-[#C9A84C]">{column.title}</h3>
                <div className="mt-4 grid gap-3">{column.links.map(([label, href]) => <Link key={`${column.title}-${href}`} href={href} className="text-sm font-semibold text-[#BFC8D9] transition hover:text-[#F2EBD8]">{label}</Link>)}</div>
              </div>
            ))}
          </div>
          <div className="rounded-3xl border border-[#C9A84C]/45 bg-[#101C3D]/80 p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-[#BFC8D9]">Ayúdanos a mejorar LEDGERA con feedback orgánico sobre claridad, utilidad y funcionamiento real de la app.</p>
            <Link href="/opinion" className="mt-5 inline-flex rounded-2xl border border-[#C9A84C]/70 px-5 py-3 text-sm font-black text-[#C9A84C] transition hover:bg-[#C9A84C] hover:text-[#080E1F]">Sugerir una mejora</Link>
          </div>
        </div>
