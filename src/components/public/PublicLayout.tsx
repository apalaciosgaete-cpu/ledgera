...
export const publicNavLinks = [
  ...
  { label: "Servicio asistido", href: "/servicio-asistido" },
      ...
      >
        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Navegación principal">
          {publicNavLinks.map((item) => (
            <PublicNavLink key={item.href} href={item.href}>{item.label}</PublicNavLink>
          ))}
          ...
          >
            {publicNavLinks.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                style={{ color: "var(--text-soft)", fontSize: 14, fontWeight: 750, textDecoration: "none" }}
              >
                {item.label}
              </Link>
            ))}
            <span style={{ background: "var(--border)", height: 1 }} />
            <Link href="/login" style={{ color: "var(--text-soft)", fontSize: 14, fontWeight: 750, textDecoration: "none" }}>
              Iniciar sesión
            </Link>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {links.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            style={{ color: publicPalette.textMuted, fontSize: 13, fontWeight: 650, lineHeight: 1.35, textDecoration: "none" }}
          >
            {link.label}
          </Link>
        ))}
      ...
      <PublicContainer>
        ...
        >
          <div>
            ...
            <p style={{ color: publicPalette.textFaint, fontSize: 13, lineHeight: 1.7, margin: "16px 0 0", maxWidth: 320 }}>
              Plataforma para ordenar operaciones cripto, resolver inconsistencias y generar respaldo tributario trazable para revisión en Chile.
            </p>
          </div>
          ...
          <FooterGroup
            ...
            links={[
              ...
              { label: "Contador para activos digitales", href: "/contador-crypto-chile" },
            ]}
          />
          <FooterGroup
            ...
            links={[
              ...
              { label: PUBLIC_CONTACT_EMAIL, href: "mailto:admin@ledgera.cl" },
            ]}
          />
        </div>
        ...
        >
          ...
          <p style={{ color: "var(--text-soft)", fontSize: 12, margin: 0 }}>
            Software de organización y respaldo tributario. No reemplaza asesoría profesional.
          </p>
        </div>
      </PublicContainer>
    </footer>
  ...
  return (
    <section
      style={{
        ...
        background: "radial-gradient(circle at top left,var(--accent-soft),transparent 34%),linear-gradient(135deg,var(--bg-elev),var(--bg))",
        padding: "86px 0 70px",
      }}
    >
      <PublicContainer>
  ...
  return (
    <Link
      ...
      style={{
        ...
        border: primary ? "1px solid var(--accent)" : `1px solid ${publicPalette.border}`,
        borderRadius: 12,
    ...
    <section style={{ background: publicPalette.page, padding: "72px 0" }}>
      <PublicContainer>
        ...
        >
          ...
          <p style={{ color: publicPalette.textMuted, fontSize: 16, lineHeight: 1.7, margin: "0 auto 26px", maxWidth: 680 }}>
            {description}
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 12, flexWrap: "wrap" }}>
            <PublicButton href={primaryHref}>{primaryLabel}</PublicButton>
            <PublicButton href={secondaryHref} variant="secondary">{secondaryLabel}</PublicButton>
          </div>
        </div>
      </PublicContainer>
    </section>
    ...
    <PublicShell>
      ...
      <section style={{ background: publicPalette.section, padding: "64px 0" }}>
        <PublicContainer style={{ maxWidth: 900 }}>
          <article style={{ display: "grid", gap: 22 }}>{children}</article>
        </PublicContainer>
      </section>
    </PublicShell>
  );
...
export function LegalSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    ...
    >
      ...
      <div style={{ color: publicPalette.textMuted, display: "grid", fontSize: 15, gap: 12, lineHeight: 1.78 }}>
        {children}
      </div>
    </section>
  );
