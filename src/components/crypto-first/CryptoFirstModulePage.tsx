...
export function CryptoFirstModulePage({ module, sections = [] }: Props) {
  ...
  return (
    <main style={{ display: "grid", gap: 20 }}>
      <section style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 22, padding: "30px 32px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 24, alignItems: "flex-start", flexWrap: "wrap" }}>
          <div>
            ...
            <p style={{ color: "#64748B", fontSize: 16, lineHeight: 1.7, maxWidth: 760, margin: 0, fontFamily: fonts.body }}>
              {module.description}
            </p>
          </div>
          <span style={{ border: "1px solid rgba(15,118,110,0.18)", background: "rgba(15,118,110,0.08)", color: "#0F766E", borderRadius: 999, padding: "8px 14px", fontSize: 13, fontWeight: 800, whiteSpace: "nowrap", fontFamily: fonts.body }}>
            {STATUS_LABEL[module.status] ?? module.status}
          </span>
        </div>
      </section>

      <section style={{ background: "#071B28", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 22, padding: "28px 30px" }}>
        ...
        <h2 style={{ color: "#F8FAFC", fontSize: "1.45rem", fontWeight: 850, margin: "0 0 18px", fontFamily: fonts.body }}>
      ...
      <section style={{ display: "grid", gap: 16, gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))" }}>
        {["Vista ejecutiva", "Detalle", "Documentos relacionados", "Próxima acción"].map((title, index) => (
          <article key={title} style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 18, padding: 22 }}>
