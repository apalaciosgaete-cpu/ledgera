...
function ButtonLink({ href, children, primary = false }: { href: string; children: React.ReactNode; primary?: boolean }) {
  return (
    <a href={href} style={{ minHeight: 34, padding: "8px 12px", borderRadius: 9, border: primary ? "1px solid #0F766E" : "1px solid #CBD5E1", background: primary ? "#0F766E" : "#FFFFFF", color: primary ? "#FFFFFF" : "#475569", fontSize: 12, fontWeight: 900, textDecoration: "none", display: "inline-flex", alignItems: "center", whiteSpace: "nowrap", fontFamily: fonts.body }}>
      {children}
    </a>
  );
}
...
function DecisionCard({ title, status, text, tone }: { title: string; status: string; text: string; tone: "green" | "amber" }) {
  const color = tone === "green" ? "#15803D" : "#B45309";
  ...
  return (
    <article style={{ ...card, border: `1px solid ${border}`, background: bg }}>
      ...
      <h3 style={{ color: "#0F2A3D", fontSize: 19, fontWeight: 950, margin: "0 0 6px", fontFamily: fonts.display }}>{status}</h3>
      <p style={{ color: "#475569", fontSize: 13.5, lineHeight: 1.45, margin: 0, fontFamily: fonts.body }}>{text}</p>
    </article>
  );
}
...
function PredictiveCard({ title, text }: { title: string; text: string }) {
  return (
    <article style={card}>
      <h3 style={{ color: "#0F2A3D", fontSize: 15, fontWeight: 900, margin: "0 0 6px", fontFamily: fonts.body }}>{title}</h3>
      <p style={{ color: "#64748B", fontSize: 13, lineHeight: 1.45, margin: 0, fontFamily: fonts.body }}>{text}</p>
    </article>
  );
}
  ...
  return (
    <main style={{ display: "grid", gap: 14, alignContent: "start" }}>
      <section style={{ padding: "0 14px 4px" }}>
        <p style={{ color: "#0F766E", fontSize: 11, fontWeight: 850, letterSpacing: "0.08em", margin: "0 0 6px", textTransform: "uppercase", fontFamily: fonts.body }}>LEDGERA</p>
        <h1 style={{ color: "#0F2A3D", fontSize: "clamp(1.3rem, 3.5vw, 1.8rem)", fontWeight: 900, margin: 0, letterSpacing: "-0.04em", fontFamily: fonts.display }}>{module.label}</h1>
      </section>
      <section style={{ display: "grid", gap: 10, gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
        {cards.map((text, index) => (
          <article key={text} style={card}>
            ...
            <p style={{ color: "#475569", fontSize: 13.5, lineHeight: 1.45, margin: "10px 0 0", fontFamily: fonts.body }}>{text}</p>
          </article>
        ))}
      </section>
    </main>
...
export function CryptoFirstModulePage({ module, sections }: Props) {
  ...
  return (
    <main style={{ display: "grid", gap: 12, alignContent: "start" }}>
      <section style={{ padding: "0 14px 4px" }}>
        <p style={{ color: "#0F766E", fontSize: 11, fontWeight: 850, letterSpacing: "0.08em", margin: "0 0 6px", textTransform: "uppercase", fontFamily: fonts.body }}>Situación tributaria</p>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
          <div>
            <h1 style={{ color: "#0F2A3D", fontSize: "clamp(1.3rem, 3.5vw, 1.8rem)", fontWeight: 900, margin: "0 0 4px", letterSpacing: "-0.04em", fontFamily: fonts.display }}>Declaración e impacto tributario</h1>
            <p style={{ color: "#64748B", fontSize: 13, lineHeight: 1.5, maxWidth: 820, margin: 0, fontFamily: fonts.body }}>
              LEDGERA separa lo que debes respaldar o declarar de lo que podría generar impuesto efectivo a pagar.
            </p>
          </div>
          <button type="button" onClick={rebuildTaxEvents} disabled={loading || rebuilding} style={{ background: loading || rebuilding ? "#94A3B8" : "#0F766E", border: "none", borderRadius: 999, color: "#FFFFFF", cursor: loading || rebuilding ? "not-allowed" : "pointer", fontFamily: fonts.body, fontSize: 13, fontWeight: 850, padding: "10px 15px" }}>
      ...
      <section style={{ background: "linear-gradient(135deg, #FFFFFF 0%, #F8FFFB 100%)", border: "1px solid #D9F5E8", borderRadius: 24, padding: 18, boxShadow: "0 14px 34px rgba(15,42,61,0.06)", display: "grid", gap: 14 }}>
        {message && <p style={{ background: "#ECFDF5", border: "1px solid #BBF7D0", borderRadius: 12, color: "#0F766E", fontSize: 13, fontFamily: fonts.body, fontWeight: 750, margin: 0, padding: "9px 12px" }}>{message}</p>}
        {error && <p style={{ background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 12, color: "#B91C1C", fontSize: 13, fontFamily: fonts.body, fontWeight: 750, margin: 0, padding: "9px 12px" }}>Error: {error}</p>}

        {loading ? (
          <p style={{ color: "#64748B", fontSize: 13, fontFamily: fonts.body, margin: 0 }}>Cargando situación tributaria…</p>
        ) : !hasConfirmed ? (
          <article style={card}>
            <h2 style={{ color: "#0F2A3D", fontSize: "clamp(1.15rem,2vw,1.45rem)", fontWeight: 950, margin: "0 0 6px", fontFamily: fonts.display }}>Aún no hay operaciones para analizar</h2>
            <p style={{ color: "#475569", fontSize: 13.5, lineHeight: 1.45, margin: 0, fontFamily: fonts.body }}>Primero carga información en Origen de Fondos, confirma movimientos en Importaciones y revisa Activos.</p>
          </article>
        ) : (
          <>
            ...
            <article style={{ ...card, border: "1px solid #BBF7D0", background: "#FFFFFF" }}>
              <p style={{ color: "#0F766E", fontSize: 11, fontWeight: 900, letterSpacing: "0.08em", margin: "0 0 6px", textTransform: "uppercase", fontFamily: fonts.body }}>IA LEDGERA dice</p>
              <h2 style={{ color: "#0F2A3D", fontSize: "clamp(1.12rem,2vw,1.35rem)", fontWeight: 950, margin: "0 0 8px", fontFamily: fonts.display }}>No pagar hoy no significa no declarar ni respaldar</h2>
              <p style={{ color: "#334155", fontSize: 13.5, lineHeight: 1.5, margin: 0, fontFamily: fonts.body }}>
              ...
              <p style={{ color: "#334155", fontSize: 13.5, lineHeight: 1.5, margin: "6px 0 0", fontFamily: fonts.body }}>
