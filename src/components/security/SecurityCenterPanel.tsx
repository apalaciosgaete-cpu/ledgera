...
export default function SecurityCenterPanel() {
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !payload?.ok || !payload?.data) {
        throw new Error(
          payload?.message ??
            "No fue posible verificar la seguridad de la cuenta.",
        );
      }
  ...
  const securityLevel = useMemo(() => {
    if (!status) {
      return { label: "Sin verificar", tone: "neutral" as StatusTone };
    }
  ...
  async function closeOtherSessions() {
    ...
    try {
      ...
      if (!response.ok || !payload?.ok) {
        throw new Error(
          payload?.message ?? "No fue posible cerrar las otras sesiones.",
        );
      }
  ...
  return (
    <div style={{ display: "grid", gap: "1rem", marginBottom: "1rem" }}>
      ...
      >
        ...
        >
          <div>
            ...
            >
              Los indicadores se verifican directamente en el servidor. Ningún
              control se marca como activo por defecto.
            </p>
          </div>
          ...
          {loading ? (
            <>
              <SecurityStatusRow
                ...
                status="Verificando"
                detail="Consultando el estado real de la cuenta."
                tone="neutral"
              />
              <SecurityStatusRow
                title="Correo electrónico"
                status="Verificando"
                detail="Consultando la verificación de identidad de acceso."
                tone="neutral"
              />
            </>
          ) : status ? (
            <>
              <SecurityStatusRow
                title="Verificación en dos pasos"
                status={
                  status.twoFactor.enabled ? "Activa" : "No configurada"
                }
                detail={
              ...
              <SecurityStatusRow
                ...
                detail={
                  status.email.verified
                    ...
                    : `${status.email.address} aún no registra verificación.`
                }
            ...
            <SecurityStatusRow
              ...
              status="Error"
              detail="La plataforma no pudo acreditar los controles de seguridad."
              tone="error"
            />
          )}
        </div>
      ...
      {status && !status.twoFactor.enabled && showSetup ? (
        <TwoFASetupPanel />
      ) : null}
      ...
      {status ? (
        ...
        >
          ...
          >
            <div>
              ...
              >
                ...
                >
                  {status.sessions.length} activa
                  {status.sessions.length === 1 ? "" : "s"}
                </span>
              </div>
              <p
                style={{
              ...
              >
                Revisa y administra los dispositivos que mantienen acceso a tu
                cuenta.
              </p>
            </div>
            ...
            {status.sessions.length > 1 ? (
              <button
                ...
                style={{
                  ...secondaryButtonStyle,
                  cursor: closingSessions ? "not-allowed" : "pointer",
                  opacity: closingSessions ? 0.55 : 1,
                }}
              >
          ...
          {status.sessions.length > 0 ? (
              ...
              {status.sessions.map((session) => (
                ...
                >
                  <div>
                    ...
                    >
                      Iniciada: {formatDateTime(session.createdAt)} · Vence:{" "}
            ...
            >
              No se encontraron sesiones vigentes.
            </div>
          )}
        </section>
      ) : null}
